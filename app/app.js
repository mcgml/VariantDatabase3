...
angular.module('variantdatabase', [ 'ngResource', 'ngRoute', 'variantdatabase.login', 'variantdatabase.report', 'variantdatabase.search', 'variantdatabase.account', 'variantdatabase.admin', 'variantdatabase.about', 'variantdatabase.qc', 'variantdatabase.test'])
    ...
    .filter('gtFilter', function() {
        return function (input) {
            if (input === "HAS_HET_VARIANT"){
                return "HET";
            } else if ("HAS_HOM_VARIANT") {
                return "HOM";
            } else {
                return "NULL";
            }
    ...
    .factory('framework', function() {
        ...
        return {
            ...
            keywordSearchFromAnnotationObject: function(element){
                var keywords = '';

                if (element.symbol.properties.hasOwnProperty('symbolId')){
                    keywords += " \"";
                    keywords += element.symbol.properties.symbolId;
                    keywords += "\"";
                }

                if (element.symbol.properties.hasOwnProperty('hgvsc')){
                    keywords += " \"";
                    keywords += element.annotation.properties.hgvsc.substring(2);
                    keywords += "\"";
                }

                if (element.symbol.properties.hasOwnProperty('hgvsp')){
                    if (element.symbol.properties.hgvsp !== "p.="){
                        keywords += " \"";
                        keywords += element.annotation.properties.hgvsp.substring(2);
                        keywords += "\"";
                    }
                }
    ...
    .controller('VariantSelectionCtrl', ['$scope', '$uibModalInstance', 'items', 'Notification', '$http', function ($scope, $uibModalInstance, items, Notification, $http) {
        ...
        $http.get('/api/variantdatabase/panel/info', {}
        ).then(function(response) {
